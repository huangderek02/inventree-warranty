 ...
 */
function WarrantyPanel({ context }: { context: InvenTreePluginContext }) {
  ...
  const apiQuery = useQuery(
    {
      ...
      queryFn: async () => {
        const url = `/plugin/warranty/example/`;
  ...
  return (
    <>
      ...
      <Stack gap='xs'>
        ...
        <Text>This is a custom panel for the Warranty plugin.</Text>
        <SimpleGrid cols={2}>
...
export function renderWarrantyPanel(context: InvenTreePluginContext) {
  checkPluginVersion(context);

  return (
    <LocalizedComponent locale={context.locale}>
      <WarrantyPanel context={context} />
    </LocalizedComponent>
  );
}
