    ...
    for row in rows:
        try:
            ...
            with transaction.atomic():
                # Prefer dedupe by audit_id; fallback to unit_sn for legacy rows
                if audit_id:
                ...
                if was_created:
                    ...
                    changed = False
                    # keep unit_sn authoritative if it already exists
                    if not obj.unit_sn and unit_sn:
                        obj.unit_sn = unit_sn; changed = True
                    if obj.model_number != model_number:
                        obj.model_number = model_number; changed = True
                    if obj.ums_sn != ums_sn:
                        obj.ums_sn = ums_sn; changed = True
                    if obj.tm_device_id != tm_id:
                        obj.tm_device_id = tm_id; changed = True
                    if obj.audit_date != audit_date:
                        obj.audit_date = audit_date; changed = True
                    if obj.warranty_expiry != warranty_expiry:
                        obj.warranty_expiry = warranty_expiry; changed = True
                    if audit_id and obj.audit_id != audit_id:
                        obj.audit_id = audit_id; changed = True
                    if mod_at and obj.sc_modified_at != mod_at:
                        obj.sc_modified_at = mod_at; changed = True
                    obj.payload = detail
                    if changed:
                        obj.save()
                        updated += 1
...
def verify_all_synced(modeladmin, request, queryset):
    try:
        ...
        msg = f"SC verify scanned={res['checked']}; see server logs for any missing audit_ids."
