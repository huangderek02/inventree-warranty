...
class Warranty(AppMixin, SettingsMixin, InvenTreePlugin):
    ...
    SCHEDULED_TASKS = {
        "warranty-sync-daily": {
            "func": "scheduled_sync_from_sc",  # method name below
            "schedule": "I",                   # interval schedule
    ...
    def scheduled_sync_from_sc(self):
        """
        Background entrypoint invoked by the worker.

        Keep imports local so the plugin remains lightweight when simply loaded.
        """
        from . import admin as warranty_admin  # late import keeps this module lean

        result = warranty_admin.run_sc_sync(print_each=False)
